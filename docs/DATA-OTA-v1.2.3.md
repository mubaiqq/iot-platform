...
- 升级期间保持稳定电源与网络，不要断电。
- 当前设备端会计算并比对下载固件 SHA-256，下载 URL 绑定目标设备；平台还会用独立私钥签署设备码、类型、版本和摘要，设备使用内置公钥验签，伪造 MQTT OTA 指令无法通过。TLS 当前仍使用兼容模式 `setInsecure()`，因此下载链路尚未达到证书校验级别；后续还应加入 CA/公钥固定，并为 MQTT 启用设备级凭据与 Topic ACL。
- OTA 签名私钥保存在运行目录 `data/ota-signing-private.pem`（不会提交 Git）；一键更新保留 `data/`，部署迁移到其他服务器时必须安全复制该私钥，并通过 `OTA_SIGNING_KEY_PATH` 指定位置。
