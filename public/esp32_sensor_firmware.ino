...
void performOTA(const String& url, const String& version, const String& expectedSha256, int versionId) {
  if (url.length() == 0 || version.length() == 0 || expectedSha256.length() != 64 || !isNewerFirmwareVersion(version)) { publishOTAStatus("failed", version, "升级参数无效或版本不高于当前版本", versionId); return; }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (doc.containsKey("command") && doc["command"] == "ota") {
    ...
    String version=doc["version"]|""; String sha256=doc["sha256"]|""; int versionId=doc["version_id"]|0; String otaSignature=doc["ota_signature"]|"";
    if(targetDevice!=deviceCode||targetType!="sensor"){publishOTAStatus("failed",version,"升级指令与当前设备不匹配",versionId);return;}
    if(!verifyOTACommandSignature(targetDevice,targetType,version,sha256,versionId,otaSignature)){publishOTAStatus("failed",version,"升级指令签名无效",versionId);return;}
